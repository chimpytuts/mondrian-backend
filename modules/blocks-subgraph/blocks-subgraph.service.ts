...
export class BlocksSubgraphService {
    ...
    constructor() {
        this.client = new GraphQLClient("https://testeborabora.cyou/subgraphs/name/blocks");
    }
    ...
    /*public async getHourlyBlocks(numDays: number): Promise<BlockFragment[]> {
        ...
        for (const timestamp of timestamps) {
            console.log("ALLLLLBLOCKS: "+timestamp)
        ...
        const args: BlocksQueryVariables = {
            ...
            where: {
                  timestamp_gte: `${moment
                    .tz('GMT')
                    .subtract(1, 'day')
                    .subtract(800 * blockTime, 'seconds')
                    .unix()}`,
                timestamp_lte: `${moment
                    .tz('GMT')
                    .subtract(1, 'day')
                    .add(800 * blockTime, 'seconds')
                    .unix()}`,                
            },
    ...
    public async getBlockForTimestamp(timestamp: number): Promise<BlockFragment> {
        const blockTime = BLOCK_TIME_MAP[env.CHAIN_ID] ?? 3;
    ...
    public async getDailyBlocks(numDays: number): Promise<BlockFragment[]> {
        ...
        console.log("BITCHHH : "+timestamps);
    ...
    public async getBlocksPerDay() {
        ...
        console.log("BlockTime: "+secondsPerDay/blockTime);
